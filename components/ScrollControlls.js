...
const ScrollControls = () => {
  ...
  const progressWidth = useTransform(progressMV, (v) => `${Math.round(v * 100)}%`);
  ...
  const bounceTransition = {
    y: { duration: 0.9, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }
  };
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const pct = docHeight > 0 ? Math.min(1, Math.max(0, scrolled / docHeight)) : 0;
      progressMV.set(pct);
