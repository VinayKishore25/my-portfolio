...
// Local fallback for fadeIn if you don't have the file
const fadeIn = (direction, delay) => ({
  ...
  show: { 
    ...
    x: 0,
    opacity: 1, 
    transition: { type: 'tween', duration: 1.1, delay: delay, ease: [0.25, 0.25, 0.25, 0.75] } 
  },
});
...
const AboutMe = () => {
  const stats = [
    ...
    { label: 'Projects Shipped', value: '42+', color: 'text-accent' }, // Using your global accent red
    { label: 'LeetCode Solved', value: '246+', color: 'text-green-400' },
  ];

  const navButtons = [
    'Skills', 'Projects', 'Resume', 'Experience', 'Feed', 'AI Companion'
  ];
        ...
        <div className="flex flex-col xl:flex-row gap-12 xl:gap-20 items-center">
                ...
                <div className="absolute bottom-0 left-0 w-full p-6 md:p-8 flex justify-between items-end border-t border-white/10 bg-black/40 backdrop-blur-md">
                    {stats.map((stat, index) => (
                        <div key={index} className="flex flex-col">
                            <span className={`text-2xl md:text-3xl font-bold font-mono ${stat.color === 'text-accent' ? 'text-[#F13024]' : stat.color}`}>
                                {stat.value}
                            </span>
          ...
          >
            ...
            <p className="text-white/70 text-lg leading-relaxed mb-6 max-w-xl">
              I am a <span className="text-white font-semibold">Artificial Intelligence and Machine Learning Student </span> at Aditya University '26, bridging the gap between complex backend logic and intuitive frontend design.
            </p>
            
            <p className="text-white/50 text-base leading-relaxed mb-12 max-w-xl font-light">
              My passion lies in <strong>Artificial Intelligence</strong> and <strong>Full Stack Development</strong>. Whether it's crushing LeetCode problems or deploying scalable applications, I build digital experiences that matter.
            </p>
